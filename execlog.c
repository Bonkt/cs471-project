...
static void vcpu_insn_exec_only_regs(unsigned int cpu_index, void *udata)
{
    ...
    if (cpu->last_exec->len) {
        ...
        // NOTE: this is where we print the 
        qemu_plugin_outs(cpu->last_exec->str);
        qemu_plugin_outs("\n");
    }
...
static void vcpu_tb_trans(qemu_plugin_id_t id, struct qemu_plugin_tb *tb)
{
    ...
    // translation block (tb) is a basic block, same as our definition.
    ...
    for (size_t i = 0; i < n_insns; i++) {
        ...
        trex_insn_metadata_t trex_insn_metadata = {0};
        skip = trex_parse_insn_to_metadata(insn_disas, &trex_insn_metadata);    
        ...
        if (skip) {
            ...
            // Write insn virtual address:
            fwrite(&insn_vaddr, sizeof(insn_vaddr), 1, trex_binary_ins_log_file);
            // Write instruction meta data
            fwrite(&trex_insn_metadata, sizeof(trex_insn_metadata), 1, trex_binary_ins_log_file);
            
            trex_instructions_simulated++;

            if (trex_instructions_simulated == 1000) {
                                           ...
                                           char **argv)
{
    ...
    const char *binary_ins_log_path = "binary_ins_log.log";
